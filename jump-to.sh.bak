...
default_editor() {
  ...
  for e in nvim vim nano vi; do have "$e" && {
    printf '%s\n' "$e"
    return
  }; done
  printf '%s\n' "vi"
...
map_types() {
  ...
  while IFS=, read -r tok; do
    ...
    ps1) out+=(ps1) ;;
    r | R) out+=(R r) ;;
    md | markdown) out+=(md markdown) ;;
    txt) out+=(txt) ;;
    json) out+=(json) ;;
    toml) out+=(toml) ;;
    yml | yaml) out+=(yml yaml) ;;
    *) out+=("$tok") ;; # treat as literal extension
    esac
  done <<<"$raw"
  printf '%s\n' "${out[@]}"
}
...
show_help() {
  cat <<'EOF'
jump-to — Navigate to a base directory (ENV or PATH), optionally search inside it, and run commands.

USAGE
  jump-to [ENV_NAME | -p PATH] [SUBPATH] [OPTIONS]

BASE RESOLUTION
  • ENV_NAME         Environment variable *name* without '$', e.g.  jump-to MY_REPOS
                     (equivalent of: cd "$MY_REPOS").
  • -p, --path PATH  Use an explicit path as the base (file or directory).
  • SUBPATH          Optional relative subdirectory under the base, e.g. "UserScripts/wrappers".
  Notes:
    - If PATH is a file, the base is its directory and the file becomes the initial selection.
    - If both ENV_NAME and -p are given, -p wins.

SEARCH / FILTER
  -n, --name NAME            Exact basename match (files or directories). If duplicates, an interactive
                             picker is shown (fzf if available, else a numbered menu).
  -f, --fzf, --fuzzy-finding Enable fuzzy selection over found matches (always used if multiple matches
                             and fzf is installed, or when -x/--execute is supplied and ambiguous).
  -t, --file-type LIST       Comma-separated extension filters, e.g. "py,sh,md" or ".py,.sh".
                             Examples: py, sh, ps1, R, md, txt, json, yml, yaml (case-insensitive).
  ...
  -d, --depth N              Limit recursion depth; implies recursion when present.
                             (depth=1 inspects immediate children only; depth=0 is the base itself.)

ACTION / EXECUTION
  -x, --execute CMD          Run CMD on the chosen target.
                               • '{}' in CMD is replaced with the path.
                               • If '{}' is absent and a file is selected, the path is appended to CMD.
                               • If target is a directory, CMD runs in that directory with no arguments.
  -e, --editor EDITOR        Editor to use for files when -x is not provided (defaults to $EDITOR or nvim|vim|nano|vi).
  -j, --jump-to              Drop into a subshell at the final directory (to "stay" there interactively).
...
while (($#)); do
  ...
  -r | --recursion)
    ...
    ;;
  -d | --depth)
    [[ $# -ge 2 && "$2" =~ ^[0-9]+$ ]] || die "depth must be a non-negative integer"
    DEPTH="$2"
    RECURSE=true
    shift 2
    ;;
  ...
  -*)
    ...
    ;;
  *)
    # First non-option token that isn't consumed by -p/--path is either ENV_NAME or SUBPATH (if -p already set and ENV_NAME empty)
...
if [[ -z "$BASE_PATH" ]]; then
  ...
  ENV_NAME="${ENV_NAME#\$}"
  [[ "$ENV_NAME" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || die "invalid ENV name: '$ENV_NAME'"
...
[[ -d "$BASE_PATH" ]] || die "base path is not a directory: $BASE_PATH"

# ───────────────── Search plan & execution ─────────────────
# If NONE of (-n, -f, -t, -r/-d, -x) are provided → simple jump (print base and optional subshell)
simple_jump=true
[[ -n "$NAME_EXACT" || "$FUZZY" = true || -n "$FILETYPES_RAW" || -n "$DEPTH" || "$RECURSE" = true || -n "$EXEC_CMD" ]] && simple_jump=false
...
if [[ -n "$FILETYPES_RAW" && "$RECURSE" = false && -z "$DEPTH" && -z "$NAME_EXACT" && "$FUZZY" = false ]]; then
...
use_fd=false
if have fd; then use_fd=true; elif have fdfind; then
  use_fd=true
...
if [[ -n "$NAME_EXACT" ]]; then
  if "$use_fd"; then
    # fd matches basename by default; anchor regex for exact match
    # Depth
    local_fd=(fd --hidden --follow --color=never --regex "^${NAME_EXACT}\$" "$BASE_PATH")
    if [[ -n "$DEPTH" ]]; then local_fd+=(--max-depth "$DEPTH"); elif [[ "$RECURSE" = false ]]; then local_fd+=(--max-depth 1); fi
...
if [[ ${#EXTS[@]} -gt 0 ]]; then
  if "$use_fd"; then
    ...
    if [[ -n "$DEPTH" ]]; then local_fd+=(--max-depth "$DEPTH"); elif [[ "$RECURSE" = false ]]; then local_fd+=(--max-depth 1); fi
    for ext in "${EXTS[@]}"; do local_fd+=(-e "$ext"); done
    add_candidates <<<"$("${local_fd[@]}" 2>/dev/null || true)"
  else
    # build -name clauses
    local_find=(find "$BASE_PATH")
    if [[ -n "$DEPTH" ]]; then local_find+=(-maxdepth "$DEPTH"); elif [[ "$RECURSE" = false ]]; then local_find+=(-maxdepth 1); fi
    local_find+=(\()
    local first=1
    for ext in "${EXTS[@]}"; do
      if ((first)); then
        ...
        first=0
      else local_find+=(-o -type f -iname "*.${ext}"); fi
    done
    local_find+=(\) -print)
...
if [[ "$FUZZY" = true && -z "$NAME_EXACT" && ${#EXTS[@]} -eq 0 ]]; then
  if "$use_fd"; then
    ...
    elif [[ "$RECURSE" = false ]]; then local_find=(find "$BASE_PATH" -type f -maxdepth 1 -print); fi
    add_candidates <<<"$("${local_find[@]}" 2>/dev/null || true)"
  fi
fi
...
if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  if [[ -n "$EXEC_CMD" && -z "$NAME_EXACT" && -z "$FILETYPES_RAW" && "$FUZZY" = false ]]; then
...
if ((${#CANDIDATES[@]} == 0)); then
  ...
  if [[ -z "$NAME_EXACT" && ${#EXTS[@]} -eq 0 && "$FUZZY" = false ]]; then
    SELECTED="$BASE_PATH"
  else
