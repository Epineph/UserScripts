...
# Home / destination time zones (edit these if your main trip changes)
HOME_TZ = "Europe/Copenhagen"
DEST_TZ = "Europe/Istanbul"

# Currency conversion: 1 DKK → TRY (set this to whatever rate you want)
DKK_TO_TRY = 4.0  # EXAMPLE value; update manually when needed.
...
class Direction:
    ...
    depart_iso: str  # "YYYY-MM-DDTHH:MM" (local time at departure airport)
    arrive_iso: str  # "YYYY-MM-DDTHH:MM" (local time at arrival airport)
...
class FlightOption:
    ...
    def out_duration(self):
        # Aware datetimes: subtraction is done in UTC → correct across time zones.
...
def format_dt_with_offset(dt: datetime) -> str:
    ...
    If dt is naive, just prints without offset.
    """
...
def price_str_with_try(price: float, currency: str) -> str:
    ...
    Assumes base entry is in DKK; prints: 'X DKK (~Y TRY)'.
    If currency is not DKK, prints just 'X CUR'.
    """
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    for f in flights:
        ...
        # Arrival expressed in departure time zone (body-clock view)
