...
def main() -> int:
  parser = argparse.ArgumentParser(
    ...
    epilog=("Downloads are recent PyPI Stats totals (day/week/month), "
            "excluding known mirrors. No lifetime totals are available.\n"
            "Ranking covers up to --candidates name matches, not all PyPI. "
            "Metadata and statistics are cached for 24 hours.")
  )
  ...
  parser.add_argument("--since", type=iso_date, metavar="YYYY-MM-DD",
                      help="keep only projects uploaded on/after this date")
  parser.add_argument("--date-field", choices=("created", "updated"),
                      default="updated", help="date used by --since")
