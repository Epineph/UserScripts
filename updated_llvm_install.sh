...
# Disk Selection (focus on internal SSDs)
selected_disks=$(lsblk -d -o NAME,SIZE,MODEL | grep -E '^nvme' | fzf -m | awk '{print "/dev/"$1}')
...
# Wait for RAID array to initialize and check if successful
if ! mdadm --detail /dev/md0; then
    echo "RAID array creation failed."
    exit 1
fi
...
pvcreate /dev/mapper/cryptarray

# Create Volume Group
vgcreate volgroup0 /dev/mapper/cryptarray
...
mount "$(echo "$selected_disks" | awk '{print $1"p1"}' | head -n1)" /mnt/boot/efi
...
sed -i 's/^HOOKS=.*$/HOOKS=(base udev autodetect modconf block mdadm_udev encrypt lvm2 filesystems keyboard fsck)/' /etc/mkinitcpio.conf

cp /etc/mkinitcpio.conf /mnt/etc/mkinitcpio.conf
cp /etc/pacman.conf /mnt/etc/pacman.conf

# Install Arch Linux
pacstrap /mnt base base-devel lvm2 mdadm cryptsetup linux linux-headers nvidia nvidia-settings nvidia-utils linux-firmware intel-ucode efibootmgr networkmanager xdg-user-dirs xdg-utils sudo nano vim mtools dosfstools python-setuptools ntfs-3g archinstall archiso arch-install-scripts
