...
Build a custom Arch ISO based on the stock "releng" profile, with:
  ...
  • Extra tools on the live ISO: fzf, lsof, strace, git, gptfdisk, bat, fd,
    reflector, rsync
  • A cleaned-up pacman.conf (official repos + Chaotic-AUR configured)
  ...
  • A helper script /usr/local/bin/setup-heini to create user "heini"
    and set passwords interactively after boot
  • A helper script /usr/local/bin/new-mirrors to regenerate mirrorlists
    via reflector
...
function install_pacman_conf() {
  ...
  echo "==> Installing cleaned pacman.conf (official + Chaotic-AUR)..."
...
# Chaotic-AUR is ENABLED here and fully configured in the ISO build:
#   - chaotic-keyring
#   - chaotic-mirrorlist
#   - keys imported and locally signed in the ISO keyring
# -----------------------------------------------------------------------------
...
# ─────────────── post-boot helper: new-mirrors via reflector ─────────
function install_new_mirrors_script() {
...
# ──────────────── configure Chaotic-AUR in the ISO root ──────────────
function install_chaotic_aur_repo() {
  echo "==> Configuring Chaotic-AUR in ISO (keys + keyring + mirrorlist)..."

  local gpgdir="${AIROOTFS}/etc/pacman.d/gnupg"
  sudo mkdir -p "$gpgdir"

  # Pacman wrapper that always operates *inside* the ISO root
  local AI_PACMAN=(
    sudo pacman
    --root "$AIROOTFS"
    --dbpath "$AIROOTFS/var/lib/pacman"
    --config "$AIROOTFS/etc/pacman.conf"
  )

  # Initialise ISO keyring on first run
  if [[ ! -d "${gpgdir}/private-keys-v1.d" ]]; then
    echo "==> Initializing ISO pacman keyring..."
    sudo pacman-key --gpgdir "$gpgdir" --init
    sudo pacman-key --gpgdir "$gpgdir" --populate archlinux
  fi

  echo "==> Importing and locally signing Chaotic-AUR keys..."
  sudo pacman-key --gpgdir "$gpgdir" -r FBA220DFC880C036
  sudo pacman-key --gpgdir "$gpgdir" --lsign-key FBA220DFC880C036

  sudo pacman-key --gpgdir "$gpgdir" --recv-key 3056513887B78AEB \
    --keyserver keyserver.ubuntu.com
  sudo pacman-key --gpgdir "$gpgdir" --lsign-key 3056513887B78AEB

  # Download chaotic keyring + mirrorlist into the host build tree
  local pkgdir="${ISO_HOME}/chaotic-pkgs"
  mkdir -p "$pkgdir"

  local keyring_pkg="${pkgdir}/chaotic-keyring.pkg.tar.zst"
  local mirrorlist_pkg="${pkgdir}/chaotic-mirrorlist.pkg.tar.zst"

  if [[ ! -f "$keyring_pkg" ]]; then
    echo "==> Downloading chaotic-keyring.pkg.tar.zst..."
    curl -L \
      'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' \
      -o "$keyring_pkg"
  fi

  if [[ ! -f "$mirrorlist_pkg" ]]; then
    echo "==> Downloading chaotic-mirrorlist.pkg.tar.zst..."
    curl -L \
      'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-mirrorlist.pkg.tar.zst' \
      -o "$mirrorlist_pkg"
  fi

  # Ensure pacman state dirs exist inside ISO root
  sudo mkdir -p "${AIROOTFS}/var/lib/pacman/sync"

  echo "==> Refreshing package databases in ISO root and installing archlinux-keyring..."
  "${AI_PACMAN[@]}" -Sy --noconfirm archlinux-keyring

  echo "==> Installing chaotic-keyring and chaotic-mirrorlist into ISO root..."
  "${AI_PACMAN[@]}" --noconfirm -U "$keyring_pkg" "$mirrorlist_pkg"
}
...
function main() {
  ...
  install_new_mirrors_script
  install_chaotic_aur_repo
  build_iso
