...
# Version: 0.4.0  |  License: MIT  |  Author: ChatGPT (adapted for Heini W.
# Johnsen)
#
# Overview
...
# From here down is your normal script. Example: a help function that uses
# helpout.
function print_help() {
  local prog
  prog=$(basename "$0")
...
detect_lang() {
  ...
  if read -r first < "$f" && [[ $first == "#!"* ]]; then
    if grep -Eq 'python(3)?(\s|$)' <<< "$first"; then
      echo python
      return
    fi
    if grep -Eq '(bash|sh|zsh)(\s|$)' <<< "$first"; then
      echo shell
      return
    fi
  fi
    ...
    if ((AUTO_INSTALL)); then
      ...
      if command -v pacman > /dev/null 2>&1; then
        ...
        case "$cmd" in
          ...
          shfmt)
            err "Please install 'shfmt' via your package manager (Go tool)"
            return 2
            ;;
        esac
      else
        err "Cannot auto-install '$label' (no pacman/pipx)."
        return 2
      fi
    else
      return 2
...
for f in "${FILES[@]}"; do
  ...
  lang=$(detect_lang "$f")
  # Provide a tiny pager helper for diffs: prefer bat, else cat.
  bat_or_cat() {
    if command -v bat > /dev/null 2>&1; then
      bat --paging=never --style=plain -l diff
    else
      cat
    fi
  }
  ...
  case "$lang" in
    python)
      ...
      python_format "$f" "$WIDTH"
      ;;
    shell)
      log "Formatting (shell): $(abspath "$f")"
      shell_format "$f" "$WIDTH" "$SHELL_INDENT"
      ;;
    *)
      warn "Unknown language for '$f' (no recognizable shebang/extension); skipping."
      ;;
  esac
done
