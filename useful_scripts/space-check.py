    ...
    if not want_system and explicit_paths:
        ...
        for p in explicit_paths:
            if str(p) == "/home" or not under(p, home):
                requires_root = True  # your rule: /home or outside $HOME ⇒ sudo required.
...
def run_du(root: Path, depth: int, xdev: bool, apparent: bool) -> List[Tuple[int, str]]:
    ...
    if xdev:
        cmd.append("-x")
    if apparent:
        cmd.append("--apparent-size")  # with -b, this is redundant but harmless
    cmd.append(str(root))
    try:
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument("--debug", action="store_true", help="Verbose errors to stderr.")
    ...
    roots, must_be_root = classify_and_require_root(args.user, args.system, explicit, home)
    ...
    for root in roots:
        try:
            if args.engine == "du":
                if not have("du"):
                    raise RuntimeError("GNU 'du' not found; use --engine python")
                eff_depth = depth if args.recursive else 0
                rows = run_du(root, eff_depth, xdev=args.xdev, apparent=args.apparent_size)
            else:
                eff_depth = depth if args.recursive else 0
