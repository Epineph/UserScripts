...
def run_system_whois(target: str, timeout: float) -> Optional[str]:
  ...
  if not exe:
    return None
  try:
    cp = subprocess.run([exe, target], capture_output=True,
...
def whois_domain(domain: str, mods, timeout: float) -> Dict[str, Any]:
    ...
    try:
      ...
      def _fmt(v):
        ...
        if isinstance(v, dt.datetime):
          return v.isoformat() if v.tzinfo else v.replace(
  ...
  if txt:
    ...
    for key in [
      ...
      "Registrar URL","Updated Date","Creation Date","Registry Expiry Date",
      "Registrar Abuse Contact Email","Registrar Abuse Contact Phone",
      "Domain Status","Name Server","DNSSEC","Registrant Organization",
      "Registrant Country","Admin Email","Tech Email",
    ]:
      m = re.findall(rf"^{re.escape(key)}:\s*(.+)$", txt,
...
def http_probe(url: str, mods, timeout: float, user_agent: str) -> Dict[str, Any]:
  out: Dict[str, Any] = {"request_url": url, "final_url": None,
...
def fetch_html(url: str, mods, timeout: float, user_agent: str,
               max_bytes: int = 512_000) -> Dict[str, Any]:
  """Fetch a bounded amount of HTML for passive analysis."""
  requests = mods.get("requests")
  out = {"url": url, "status_code": None, "content_type": None,
         "length": None, "text": None, "error": None}
  if requests is None:
    return out
  try:
...
def tls_probe(host: str, port: int, timeout: float) -> Dict[str, Any]:
  out: Dict[str, Any] = {"host": host, "port": port, "cert": None,
                         "protocol": None, "cipher": None,
  ...
  try:
    ...
    with socket.create_connection((host, port), timeout=timeout) as sock:
      with ctx.wrap_socket(sock, server_hostname=server_hostname) as ssock:
        ...
        if not_after:
          try:
            exp = dt.datetime.strptime(not_after, "%b %d %H:%M:%S %Y %Z").replace(
...
def run_wappalyzer(url: str, timeout: float) -> Optional[Dict[str, Any]]:
  ...
  if not exe:
    return None
  try:
    cp = subprocess.run([exe, url, "-J", "-t", "30"], capture_output=True,
                        text=True, timeout=timeout)
    if cp.returncode == 0 and cp.stdout.strip():
      last_obj = None
...
# ───────────────────────────────────────────────────────────────────────────────
def analyze_security_headers(headers: Dict[str, str],
                             set_cookie_list: Optional[List[str]] = None
                             ) -> Dict[str, Any]:
  ...
  if present("strict-transport-security"):
    ...
    if "preload" not in low:
      info["hsts_preload_hint"] = ("Eligible for preload only with 'preload' and "
                                   "long max-age.")
  ...
  if not rp:
    info["referrer_policy_note"] = ("Missing Referrer-Policy "
                                    "(consider 'strict-origin-when-cross-origin' or stricter).")
  elif rp.strip().lower() in {"no-referrer-when-downgrade", "unsafe-url"}:
    info["referrer_policy_note"] = (f"Referrer-Policy is '{rp}' "
                                    f"(weaker than recommended).")
...
def analyze_email_posture(domain: str, dns_records: Dict[str, Any], mods,
  ...
  lookups = 0
  for token in re.findall(r"(?::|^|\s)(include:[^\s]+|a\b|mx\b|exists:[^\s]+|ptr\b|"
                          r"redirect=\S+)", spf_joined, flags=re.I):
    kind = token.split(":")[0].split("=")[0].lower()
    if kind in {"include", "a", "mx", "exists", "ptr", "redirect"}:
      lookups += 1
  if lookups > 10:
    out["issues"].append(f"SPF may exceed the 10-DNS-lookup limit (estimated {lookups}).")
  if not spf_txt:
...
def probe_endpoint(url: str, mods, timeout: float, user_agent: str,
                   method: str = "HEAD") -> Dict[str, Any]:
  requests = mods.get("requests")
  ...
  try:
    ...
    if method == "HEAD":
      ...
      r = requests.request(method, url, timeout=timeout, headers=h,
                           allow_redirects=True)
    out["status"] = r.status_code
    out["length"] = int(r.headers.get("Content-Length", "0")) if r.headers.get(
...
def detect_wordpress(url: str, base_html: Optional[str], mods, timeout: float,
  ...
  if base_html:
    ...
    out["themes"] = p["themes"]
    if (p["core_version"] or "/wp-content/" in base_html or "/wp-includes/" in base_html
        or (p["generator"] and "wordpress" in p["generator"].lower())):
      out["detected"] = True

  # REST API root
  rest = probe_endpoint(urljoin(url, "/wp-json/"), mods, timeout, user_agent,
                        method="GET")
  ...
  if rest.get("status") in (200, 401, 403):
    ...
    try:
      ...
      if requests and rest.get("status") == 200:
        r = requests.get(urljoin(url, "/wp-json/"), timeout=timeout,
                         headers={"User-Agent": user_agent})
        if r.ok:
          j = r.json()
          out["rest_root_sample"] = {k: j.get(k) for k in
                                     ["name", "description", "url", "home"] if k in j}
    except Exception:
      pass

  # readme.html
  readme = probe_endpoint(urljoin(url, "/readme.html"), mods, timeout, user_agent,
                          method="GET")
  ...
  admin_ajax = probe_endpoint(urljoin(url, "/wp-admin/admin-ajax.php"), mods,
                              timeout, user_agent)
  ...
  # xmlrpc
  xmlrpc = probe_endpoint(urljoin(url, "/xmlrpc.php"), mods, timeout, user_agent,
                          method="GET")
  ...
  # uploads dir listing
  uploads = probe_endpoint(urljoin(url, "/wp-content/uploads/"), mods, timeout,
                           user_agent, method="GET")
...
def allowed_methods(url: str, mods, timeout: float, user_agent: str) -> Dict[str, Any]:
  ...
  try:
    requests = mods.get("requests")
    if requests:
      tr = requests.request("TRACE", url, timeout=timeout,
                            headers={"User-Agent": user_agent})
...
def check_sensitive_files(url: str, mods, timeout: float, user_agent: str
                          ) -> List[Dict[str, Any]]:
  candidates = [
    "/wp-config.php", "/wp-config.php.bak", "/wp-config.php~", "/wp-config.zip",
    "/wp-config.tar.gz", "/backup.zip", "/backup.tar.gz", "/database.sql", "/.env",
    "/.git/config", "/.git/HEAD", "/.gitignore", "/.svn/entries", "/.DS_Store",
    "/.user.ini", "/phpinfo.php", "/wp-config-sample.php", "/wp-content/debug.log",
...
# ───────────────────────────────────────────────────────────────────────────────
def wpscan_lookup(kind: str, key: str, token: str, mods,
...
def enrich_with_wpscan(token: str, wp_info: Dict[str, Any], mods,
                       timeout: float) -> Dict[str, Any]:
...
def _pw_capture(urls: List[str],
                outdir: Path,
                har: bool,
                screens: bool,
                pdf: bool,
                timeout_ms: int,
                axe_js: Optional[str]) -> Dict[str, Any]:
  """
  Playwright evidence capture. Returns a dict with artifact paths and page info.
  """
  result: Dict[str, Any] = {"artifacts_dir": None, "pages": [], "har": None,
                            "errors": []}
  try:
  ...
  try:
    with sync_playwright() as p:
      ...
      for i, u in enumerate(urls, 1):
        ...
        if axe_js:
          try:
            ...
            axe_file.write_text(json.dumps(axe_res, indent=2), encoding="utf-8")
          except Exception as e:
      ...
      try:
        ...
        cookies_out.write_text(json.dumps(context.cookies(), indent=2),
                               encoding="utf-8")
        storage_state = context.storage_state()
        Path(storage_out).write_text(json.dumps(storage_state, indent=2),
                                     encoding="utf-8")
...
def _print_telemetry(report: Dict[str, Any]) -> None:
  try:
    ...
    if wp and wp.get("detected"):
      _e(f"  WordPress detected; core≈{wp.get('core_version_hint')} "
    ...
    if pw and not pw.get("error"):
      ...
      _e(f"  Playwright: {len(pages)} page(s), HAR={'yes' if pw.get('har') else 'no'}")
  except Exception:
    pass
...
"""
  p = argparse.ArgumentParser(
    ...
    description=("Audit a URL: DNS/WHOIS/RDAP, HTTP/TLS, security headers, email "
  ...
  p.add_argument("--no-vuln", action="store_true",
                 help="Disable passive vulnerability checks.")
  p.add_argument("--plugin-readmes", action="store_true",
                 help="Lightly probe plugin readme.txt for version hints.")
  p.add_argument("--wpscan-api-token", default="",
                 help="WPScan API token for CVE enrichment (optional).")

  # Playwright capture
  p.add_argument("--pw", action="store_true",
                 help="Enable Playwright evidence capture.")
  p.add_argument("--pw-har", action="store_true",
                 help="Record a minimal HAR for the session.")
  p.add_argument("--pw-screens", action="store_true",
                 help="Take full-page screenshots.")
  p.add_argument("--pw-pdf", action="store_true",
                 help="Export printable PDF (Chromium build permitting).")
  p.add_argument("--pw-timeout-ms", type=int, default=20000,
                 help="Per-page navigation timeout in ms (default: 20000).")
  p.add_argument("--pw-axe", action="store_true",
                 help="Try to auto-resolve and run axe-core.")
  p.add_argument("--pw-axe-js", metavar="PATH", default=None,
                 help="Explicit path to axe.min.js (overrides auto-resolve).")
  p.add_argument("--evidence-dir", metavar="DIR", default=None,
...
def main(argv: Optional[List[str]] = None) -> int:
  ...
  if not args.no_wappalyzer:
    try:
      ...
      if w is None:
        report["errors"]["wappalyzer"] = ("Wappalyzer CLI not found or failed; "
  ...
  if not args.no_vuln:
    ...
    try:
      ...
      fetched = fetch_html(fin_url, mods, args.timeout, args.user_agent,
                           max_bytes=512_000)
      if fetched.get("text"):
    ...
    try:
      vuln["sensitive_files"] = check_sensitive_files(
        ...
        mods, args.timeout, args.user_agent)
    except Exception as e:
      ...
      if args.plugin_readmes and wp_info and wp_info.get("plugins"):
        ...
        if requests:
          for slug in list(wp_info["plugins"].keys())[:25]:
            u = urljoin((http_info.get("final_url") or url) if http_info else url,
                        f"/wp-content/plugins/{slug}/readme.txt")
            try:
              r = requests.get(u, timeout=args.timeout,
                               headers={"User-Agent": args.user_agent,
                                        "Range": "bytes=0-2048"})
              if r.status_code in (200, 206) and "text" in r.headers.get("Content-Type","").lower():
                m = re.search(r"(?im)^\s*Stable\s+tag:\s*([0-9][0-9.\-a-zA-Z]+)\s*$",
                              r.text)
                details[slug] = {"status": r.status_code,
    ...
    try:
      ...
      if token and wp_info and (wp_info.get("detected")
                                or wp_info.get("plugins") or wp_info.get("themes")):
  ...
  if args.pw:
    try:
      ...
      axe_path = resolve_axe_js_path(args.pw_axe_js) if (args.pw_axe or args.pw_axe_js) else None
      # Output directory
      evdir = Path(args.evidence_dir) if args.evidence_dir else Path(
